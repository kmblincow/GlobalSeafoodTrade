...
#Aquaculture Product Removal

#The purpose of this script is to generate a data set that removes products
#likely associated with aquaculture from our analysis, and then re-run it see 
#if we get substantially different results.


#The purpose of this script is to work with the GTA data to:
# 1. filter out non-food use products
# 2. filter out potential aquaculture-derived products
...
#aquaculture removal info
aqua <- read.csv("ConsumptionAngle/AquacultureRemovalProductCodes.csv")
...
#filter out the aquaculture products
toberemoved <- left_join(dt, aqua, by = join_by(Description == Product_Description)) 
dt_wild <- toberemoved[is.na(toberemoved$Product.y),]

dt_wild <- select(dt_wild, -Product.y) %>% rename(Product = Product.x)
...
####exporting to csv####
write.csv(dt2, "ConsumptionAngle/GTA_FAOmatch_Wild.csv")
