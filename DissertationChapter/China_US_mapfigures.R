...
#The purpose of this script is to generate the China/US map figures
...
US_shapes <- geom_polygon(aes(x = long, y = lat, group = group),
                          data = mapd_US, color = "gray30", fill = "#005AB5",
                          size = 0.15, alpha = 0.9) 
...
US_map <- ggplot(geodata) + country_shapes + US_shapes +
  ...
  scale_color_gradient(name = expression(FMI[P]),
                       na.value = "grey50", low = "#DC3220", high = "#005AB5") +
  scale_fill_gradient(name = expression(FMI[P]),
                      na.value = "grey50", low = "#DC3220", high = "#005AB5") +
  labs(title = "Imports to the United States") +
  mapcoords + maptheme

#plot origin of US imports
...
p2 <- ggplot(US2, aes(x = Top, y = propI)) +
  ...
  annotate("text", x = 22, y = 0.24, label = expression(paste("US ",FMI[P],"= 0.932"))) +
  theme_bw() +
...
png(filename="ConsumptionAngle/Figures/USImportsMap.png", 
    units="in", 
    width=15, 
    height=10, 
    pointsize=15, 
...
Ch_shapes <- geom_polygon(aes(x = long, y = lat, group = group),
                          data = mapd_Ch, color = "gray30", fill = "#d23930",
                          size = 0.15, alpha = 0.9) 
...
p3 <- ggplot(Ch2, aes(x = Top, y = propE)) +
  ...
  annotate("text", x = 22, y = 0.23, label = expression(paste("China ",FMI[P],"= 0.370"))) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 35, hjust = 1),
        legend.position = "none") 
...
png(filename="ConsumptionAngle/Figures/ChExportsMap.png", 
    units="in", 
    width=15, 
    height=10, 
    pointsize=15, 
