...
#The purpose of this script is to address the third question in the trade analysis.
...
GTA_imp <- GTA %>% filter(Direction == "Import") %>% 
  group_by(Reporter) %>% 
  summarize(Importsports = sum(livewgt, na.rm = T),
            Partners = n_distinct(Partner))
...
p <- ggplot(d3) +
  ...
  labs(x = expression(FMI[P]), y = "Proportion of Global Exports", 
...
d4 <- left_join(d, GTA_imp, by = c("Country" = "Reporter")) %>% 
  mutate(ImportsTot = sum(Imports),
         ImportProp = Imports/ImportsTot,
         diff = FMIp - mean(c(FMIc1, FMIc2, FMIc3))) %>% 
  arrange(FMIp)
...
p2 <- ggplot(d4) +
  geom_point(aes(x = FMIp, y = ImportProp, 
                 color = Partners, size = Partners)) +
  labs(x = expression(FMI[P]), y = "Proportion of Global Imports", 
       size = "# of Trade Partners",
       color = "# of Trade Partners") +
  scale_color_continuous(limits=c(2, 217), breaks=seq(50, 200, by=50)) +
  guides(color= guide_legend(), size=guide_legend()) +
